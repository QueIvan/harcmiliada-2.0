...
public class GameController {
    ...
    public List<GameDto> getAllGamesByOwnerId(@PathVariable("ownerId") UUID ownerId, @PathVariable("userId") UUID userId) {
        return facade.getAllGamesByOwnerId(ownerId, userId);
    }

    @GetMapping(value="/{gameId}/{userId}", produces = APPLICATION_JSON_VALUE)
    public GameDto getGameById(@PathVariable("gameId") UUID gameId, @PathVariable("userId") UUID userId) {
        return facade.getGameById(gameId, userId);
    }

    @GetMapping(value = "/{userId}", produces = APPLICATION_JSON_VALUE)
    public List<GameDto> getAllGames(@PathVariable("userId") UUID userId) {
        return facade.getAllGames(userId);
    }

    @PostMapping(value="/{userId}", consumes = APPLICATION_JSON_VALUE, produces = APPLICATION_JSON_VALUE)
    public GameDto createGame(@RequestBody GameDto game, @PathVariable("userId") UUID userId) {
    ...
    public GameDto updateGame(@RequestBody GameDto game, @PathVariable("userId") UUID userId) {
        return facade.updateGame(game, userId);
    }

    @DeleteMapping(value = "/{gameId}/{userId}")
    public void deleteGame(@PathVariable("gameId") UUID gameId, @PathVariable("userId") UUID userId) {
