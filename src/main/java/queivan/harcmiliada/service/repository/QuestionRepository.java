...
public interface QuestionRepository extends JpaRepository<Question, UUID> {
    ...
    @Query(value = "select * from _questions where id not in (select questions_id from _games_questions where games_id = ?1) AND (creator_id = ?2 OR is_public = true)", nativeQuery = true)
